...
const Home: NextPage = () => {
  ...
  const contractAddress = "0xB6a9Eb318aa29BF9E91dd9eC0c5C041B772701a0";
  ...
  return (
    <Container maxW={"1200px"} w={"full"}>
      <Flex justifyContent={"space-between"} alignItems={"center"} py={"20px"} height={"80px"}>
        <Box>
          <Text fontWeight={"bold"} color={"purple"}>Blick</Text>
        </Box>
        <ConnectWallet />
      </Flex>
      <SimpleGrid columns={2} spacing={10} mt={"40px"}>
        <Box>
          <Card>
            <CardBody>
              <Heading mb={"20px"}>Place for your donats</Heading>
              <Flex direction={"row"}>
                <Text>Total donats on website: </Text>
                <Skeleton isLoaded={!loadingTotalAdds} width={"20px"} ml={"5px"}>
                  {totalAdds?.toString()}
                </Skeleton>
              </Flex>
              <Text fontSize={"2xl"} py={"10px"}>Name: </Text>
              <Input placeholder="Ihor" maxLength={16} value={name} onChange={handleNameChange}></Input>
              ...
              <Input placeholder="https://imagebobmer/image.png" maxLength={300} value={imageUrl} onChange={handleImageUrlChange}></Input>
        ...
        <Box>
          <Card maxH={"60vh"} overflow={"scroll"}>
            <CardBody>
              <Text fontWeight={"bold"}>Recent Donats:</Text>
              {!loadingAllAdds ? (
                <Box>
